...
namespace BL.Services
{
    public class ShipmentService : BaseService<TbShipment, ShipmentDto>, IShipment
    {
        ...
        public ShipmentService(IGenericRepository<TbShipment> repo, IMapper mapper,
        ...
        public async Task<PagedResult<ShipmentDto>> GetShipments(int pageNumber, int pageSize, bool isUserRole, int? status)
        {
            try
            {
                var userId = _userService.GetLoggedInUser();
                ...
                var result = await _repo.GetPagedList(
                    ...
                    pageSize: pageSize,
                    filter: a => (a.CreatedBy == userId || (!isUserRole)) && (a.CurrentState == status || status == null) && (a.CurrentState > 0),
                    selector: a => new ShipmentDto
                    {
                        Id = a.Id,
                        ShipingDate = a.ShipingDate,
        ...
        public async Task<PagedResult<ShipmentDto>> GetAllShipments(int pageNumber, int pageSize, bool isUserRole, int? status)
        {
            try
            {
                var userId = _userService.GetLoggedInUser();
                ...
                var result = await _repo.GetPagedList(
                    pageNumber: pageNumber,
                    pageSize: pageSize,
                    filter: a => (!isUserRole||a.CreatedBy==userId) && (status == null || a.CurrentState == status),
                    selector: a => new ShipmentDto
                    {
                        Id = a.Id,
                        ShipingDate = a.ShipingDate,
